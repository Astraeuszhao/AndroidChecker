...
}`n#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
...
}`npub struct ServiceMonitor {
    client: AdbClient,
}`nimpl ServiceMonitor {
    pub fn new(client: AdbClient) -> Self {
        Self { client }
    }`n    pub async fn list_services(&self, serial: &str) -> Result<Vec<ServiceInfo>> {
        let output = self
            .client
            .invokeShellCommand(serial, &["dumpsys", "activity", "services"])
            .await
            .context("Failed to get service list")?;`n        let services = self.parse_services(&output);
        Ok(services)
    }`n    pub async fn list_running_services(&self, serial: &str) -> Result<Vec<ServiceInfo>> {
        let all_services = self.list_services(serial).await?;
        Ok(all_services
            .into_iter()
            .filter(|s| s.state == ServiceState::Running)
            .collect())
    }`n    pub async fn get_service_details(&self, serial: &str, service_name: &str) -> Result<String> {
        let output = self
            .client
            .invokeShellCommand(serial, &["dumpsys", service_name])
            .await
            .context("Failed to get service details")?;`n        Ok(output)
    }`n    pub async fn stop_service(&self, serial: &str, package: &str, service_class: &str) -> Result<()> {
        ...
        Ok(())
    }`n    pub async fn start_service(&self, serial: &str, package: &str, service_class: &str) -> Result<()> {
        let intent = format!("{}/{}", package, service_class);
        self.client
        ...
        Ok(())
    }`n    pub async fn list_system_services(&self, serial: &str) -> Result<Vec<String>> {
        let output = self
            .client
            .invokeShellCommand(serial, &["service", "list"])
            .await
            .context("Failed to get system services")?;`n        let services: Vec<String> = output
            .lines()
            .filter_map(|line| {
            ...
            .collect();`n        Ok(services)
    }`n    fn parse_services(&self, output: &str) -> Vec<ServiceInfo> {
        let mut services = Vec::new();
        let mut current_service: Option<ServiceInfo> = None;`n        for line in output.lines() {
            let trimmed = line.trim();`n            if trimmed.starts_with("* ServiceRecord{") {
                if let Some(service) = current_service.take() {
                    services.push(service);
                }`n                if let Some(name_part) = trimmed.split_whitespace().last() {
        ...
        }`n        if let Some(service) = current_service {
            services.push(service);
        }`n        services
    }
}`n