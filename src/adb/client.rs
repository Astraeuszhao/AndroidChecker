...
pub struct AdbClient {
    ...
    pub fn new() -> Result<Self> {
        ...
        Ok(Self { adb_path })
    }`n    fn resolve_adb() -> Result<PathBuf> {
        ...
        }`n        let local_vendor = PathBuf::from("vendor/platform-tools/adb.exe");
        if local_vendor.exists() {
             if let Ok(abs) = std::fs::canonicalize(local_vendor) {
                 return Ok(abs);
             }
        }`n        if let Ok(output) = std::process::Command::new("where").arg("adb").output() {
        ...
        }`n        Err(AdbError::AdbNotFound)
    }`n    pub async fn ensure_present(&self) -> Result<()> {
        let output = timeout(
            Duration::from_secs(10),
            Command::new(&self.adb_path).arg("version").output(),
        )
        .await
        .map_err(|_| AdbError::Timeout(10))??;`n        if output.status.success() {
            Ok(())
        } else {
            Err(AdbError::CommandFailed("adb version failed".to_string()))
        }
    }`n    pub async fn start_server(&self) -> Result<()> {
        timeout(
            Duration::from_secs(10),
        ...
        .map_err(|_| AdbError::Timeout(10))??;
        Ok(())
    }`n    pub async fn execute(&self, args: &[&str]) -> Result<(String, String)> {
        let output = Command::new(&self.adb_path).args(args).output().await?;`n        let stdout = String::from_utf8(output.stdout)?;
        let stderr = String::from_utf8(output.stderr)?;`n        if !output.status.success() {
            return Err(AdbError::CommandFailed(stderr.clone()));
        }`n        Ok((stdout, stderr))
    }`n    pub async fn try_execute(&self, args: &[&str]) -> Result<(bool, String, String)> {
        let output = Command::new(&self.adb_path).args(args).output().await?;`n        let stdout = String::from_utf8_lossy(&output.stdout).to_string();
        let stderr = String::from_utf8_lossy(&output.stderr).to_string();`n        Ok((output.status.success(), stdout, stderr))
    }`n    pub async fn shell(&self, serial: &str, cmd: &[&str]) -> Result<String> {
        let mut args = vec!["-s", serial, "shell"];
        args.extend_from_slice(cmd);`n        let (stdout, stderr) = self.execute(&args).await?;`n        if !stderr.is_empty() && stderr.to_lowercase().contains("error") {
            return Err(AdbError::CommandFailed(stderr));
        }`n        Ok(stdout)
    }`n    pub async fn try_shell(&self, serial: &str, cmd: &[&str]) -> Result<(bool, String, String)> {
        let mut args = vec!["-s", serial, "shell"];
        args.extend_from_slice(cmd);
        self.try_execute(&args).await
    }`n    pub async fn logcat_stream(
        &self,
        serial: &str,
        mut callback: impl FnMut(String) + Send + 'static,
    ) -> Result<tokio::task::JoinHandle<()>> {
        let adb_path = self.adb_path.clone();
        let serial = serial.to_string();`n        let handle = tokio::spawn(async move {
            ...
            };`n            if let Some(stdout) = child.stdout.take() {
                let reader = BufReader::new(stdout);
                let mut lines = reader.lines();`n                while let Ok(Some(line)) = lines.next_line().await {
                    callback(line);
                }
            }`n            let _ = child.wait().await;
        });`n        Ok(handle)
    }`n    pub async fn parallel_shell(&self, serial: &str, commands: Vec<Vec<String>>) -> Vec<Result<String>> {
